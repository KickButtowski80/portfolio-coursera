    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      sectionVisibility.set(entry.target.id, {
       ...
       if (mostVisibleSection) {
        ...
        const mostVisibleSectionMenuItems = document.querySelectorAll(`a[href="#${mostVisibleSection}"]`);
        
        if (mostVisibleSectionMenuItems) {
            mostVisibleSectionMenuItems.forEach(
              menuLink => menuLink.classList.add('active'));
        }
    }
    }, options);
